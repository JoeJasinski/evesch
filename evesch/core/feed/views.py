...
def org_rss(request, org_short_name, org_feed_hash):
    try:
        ...
        orgfeed = feedgenerator.Rss201rev2Feed(
            title=current_org.org_name,
            link="http://{}{}".format(
                host, reverse(
                    'event_events_list',
                    kwargs={'org_short_name': current_org.org_short_name,})),
            description=current_org.org_desc,
            language='en')
        ...
        for event in events:
            orgfeed.add_item(
                ...
                description="Event on: {} --  Description: {}".format(
                    event.event_date.strftime('%d %b %Y'), event.event_desc),
...
def org_ics(request, org_short_name, org_feed_hash):
    ...
    for event in events:
        ...
        organizer.params['cn'] = vText("{} {}".format(
            event.event_creator_name.first_name,
            event.event_creator_name.last_name))
