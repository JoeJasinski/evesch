...
def group_add(request, org_short_name, template_name=None):
    ...
    if not message:
        if not current_org.is_member(request.user):
            ...
            message = Message(
                ...
                text=_("You cannot add a group in an organization that you do not belong to."))
    ...
    if not message:
        ...
        if not operms['can_add_group']:
            ...
            message = Message(
                title=_("Cannot Add Group"),
                text=_("You do not have permission to add a group in this organization."))
            message.addlink(_("Back"), current_org.get_absolute_url())
            context = {'message': message}
    if not message:
        show_dialog = False
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                message.addlink(
                    _("View"), new_group.get_absolute_url())
                message.addlink(
                    _("Edit"),
                    reverse(
                        'egroup_group_edit',
...
def group_edit(request, org_short_name, group_hash, template_name=None):
    ...
    if not message:
        if not current_org.is_member(request.user):
            ...
            message = Message(
                ...
                text=_("You cannot edit a group in an organization that you do not belong to."))
            message.addlink(
                _("Back"),
    ...
    if not message:
        ...
        if not operms['can_edit_group']:
            ...
            message = Message(
                title=_("Cannot Edit Group"),
                text=_("You do not have permission to edit a group in this organization."))
            message.addlink(
                _("Back"),
                current_org.get_absolute_url())
            context = {'message': message}
    if not message:
        ...
        context = {
            ...
            'groupusers': groupuser_page,
            'message': message,
            'show_dialog': show_dialog,
            'ajax_page_members': reverse(
                'egroup_group_user_list_ajax',
                kwargs={'org_short_name': current_org.org_short_name,
                        'group_hash': current_usergroup.group_hash})}
    else:
        template_name = "core/message.html"
    ...
    if not message:
        ...
        if not operms['is_memberof_org']:
            ...
            message = Message(
                ...
                text=_("You cannot remove a group in an organization that you do not belong to."))
            message.addlink(_("Back"), current_org.get_absolute_url())
            context = {'message': message}
    if not message:
        if not operms['can_remove_group']:
            ...
            message = Message(
                ...
                text=_("You do not have permission to remove a group in this organization."))
            message.addlink(_("Back"), current_org.get_absolute_url())
            context = {'message': message}
    if not message:
        if request.method == 'POST':
...
def groupuser_remove(request, org_short_name, group_hash, group_user, template_name=None):
    ...
    if not message:
        ...
        if not operms['is_memberof_org']:
            ...
            message = Message(
                title=_("Cannot Remove Group"),
                text=_("You cannot remove a group member in an organization that you do not belong to."))
            message.addlink(_("Back"), current_org.get_absolute_url())
            context = {'message': message}
    if not message:
        if not operms['can_remove_groupmember']:
            ...
            message = Message(
                title=_("Cannot Remove Event Type"),
                text=_("You do not have permission to remove a group member in this organization."))
            message.addlink(_("Back"), current_org.get_absolute_url())
            context = {'message': message}
    if not message:
