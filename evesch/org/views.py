...
from evesch.org.forms import (
    OrganizationForm, OrganizationFormEdit,
    OrganizationJoinForm, OrganizationInviteMember)
...
def orgs_list_all(request, template_name=None):
    ...
    if not message:
        ...
        context = {
            'all_orgs_page': all_orgs_page,
            'ajax_page_all': reverse(
                'org_orgs_list_all_ajax', kwargs={})}
    else:
        template_name = "core/message.html"
        context = {'message': message}
    return render(request, template_name, context)
...
def orgs_list_my(request, template_name=None):
    ...
    if not message:
        ...
        context = {
            'my_orgs_page': my_orgs_page,
            'ajax_page_my': reverse(
                'org_orgs_list_my_ajax', kwargs={})}
    else:
        template_name = "core/message.html"
        context = {'message': message}
    return render(request, template_name, context)
...
@login_required
@org_required
def org_join(request, current_org, template_name=None, message=None):
    current_user, message = get_current_user(request.user)
    if not message:
    ...
    if not message:
        if not operms['can_join_org']:
            ...
            message = Message(
                ...
                text=_("In order to join this organization, you need approval from the organization admin."))
            message.addlink(
                _("Back"),
                current_org.get_absolute_url())
            context = {'message': message}
    if not message:
        if request.method == 'POST':
            ...
            template_name = "core/message.html"
            message = Message(
                title=_("You have Joined the organization"),
                text=_("Org Join Successful: {}".format(current_org.org_name)))
            message.addlink(
...
@login_required
@org_required
def org_leave(request, current_org, template_name=None, message=None):
    current_user, message = get_current_user(request.user)
    if not message:
        operms = current_org.org_perms(current_user)
        if operms['is_memberof_org']:
            if request.method == 'POST':
                ...
                template_name = "core/message.html"
                message = Message(
                    title=_("Left Organization"),
                    text=_("You have left the Organization"))
                message.addlink(
                    _("Continue"),
...
@login_required
@org_required
def org_view(request, current_org, template_name=None, message=None):
    ...
    if not message:
        ...
        context = {
            ...
            'org_eventtypes': org_eventtypes,
            'members': members_page,
            'ajax_page_members': reverse(
                'org_org_user_list_ajax',
...
@login_required
@org_required
def org_members(request, current_org, template_name=None, message=None):
    if not message:
        current_user, message = get_current_user(request.user)
    if not message:
        operms = current_org.org_perms(current_user)
        if not operms['is_memberof_org']:
            ...
            message = Message(
                ...
                text=_("You cannot view members of an organization that you do not belong to."))
            message.addlink(
                _("Back"),
                current_org.get_absolute_url())
            context = {'message': message}
...
@login_required
@org_required
def org_edit(request, current_org, template_name=None, message=None):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if request.POST.get("dialog", '') == "False":
                    ...
                    show_dialog = True
            context = {'org_short_name': current_org.org_short_name,
                       'form': form,
                       'current_org': current_org,
                       'message': message,
...
@login_required
@org_required
def org_remove(request, current_org, template_name=None, message=None):
...
@login_required
@org_required
def org_member_remove(request, current_org, username=None, template_name=None, message=None):
    current_user, message = get_current_user(request.user)
    if not message:
...
@login_required
@org_required
def org_member_invite(request, current_org, template_name=None, message=None):
    current_user, message = get_current_user(request.user)
    ...
    if not message:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                new_invited_users = get_user_model().objects.filter(
                    username__in=new_user_list).exclude(user_invites_set__in=org_invites)
...
@org_required
def org_list_invites(request, current_org, template_name=None, message=None):
    invited_users_page = ePage(1)
    message = None
